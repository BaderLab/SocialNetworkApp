...
public class ChartVisualStyle extends BaseAcademiaVisualStyle {
	...
	public ChartVisualStyle(
		...
		super(applicationManager, network, socialNetwork, visualStyleFactory, passthroughMappingFactory,
				continuousMappingFactory, discreteMappingFactory, isChart);
	}
	...
	protected void applyNodeLabelPosition(VisualStyle visualStyle) {
		...
		var lexicon = applicationManager.getCurrentRenderingEngine().getVisualLexicon();
		...
		if (vp != null) {
			...
			var position = vp.parseSerializableString("N,S,c,0.00,-5.00");
			
			// If the parsed value is ok, apply it to the visual style as default value or a visual mapping
			if (position != null)
